...
**▶ [ブラウザで遊ぶ](https://shunshun0904.github.io/redplanet/)**

![スクリーンショット](docs/screenshot.png)

## 遊びかた

上のリンクを開けばそのまま遊べます。手元で動かす場合も、`index.html` を
ブラウザで開くだけです。ビルドもインストールも、サーバーも不要です。
...
- **手続き的に合成した効果音**（Web Audio API）。音声ファイルは 1 つも含まれていないため、
  読み込みは一瞬です（音のための通信は起きません）。効果音・環境音は個別にオン／オフできます。
...
- **画面は 2 列で、下に操作の帯を敷きません**。火星が縦いっぱいを使えるので盤面が大きく、
  スクロールする場所も操作盤の 1 か所だけです。狭い画面では 1 列に畳まれ、
  いま何を選べばいいかの指示だけが上端に貼りついて残ります。
...
解決の順は **9 → 1 のカウントダウン**（発射までの秒読み）です。つまり
**番号が小さいカードほど、後に解決します**。宇宙船を破壊する工作員（5）、
宇宙飛行士を排除する軍人（2）、行き先を書き換えるパイロット（1）といった妨害役が
小さい番号に並んでいるのは、他のプレイヤーが乗り込み終えたのを見てから動けるようにするためです。
第2/3版のルールブック（"countdown to blastoff" … "starting with the number 9 (the Recruiter)"、
"in descending order"）に従っています。
...
4人戦・**各400局**・席順ローテーション。互角の線は **.250** です。
括弧内は正規近似の95%信頼区間。

| | 勝率 |
|---|---|
| **AI 1人 対 素人筋 3人** | **.823** (±.037) |
| AI 1人 対 乱択 3人 | .930 (±.025) |
| 素人筋 1人 対 AI 3人 | .065 (±.024) |
| 乱択 1人 対 AI 3人 | .025 (±.015) |
| ［参考］素人筋 1人 対 乱択 3人 | .435 (±.049) |

比較対象は 2 つです。**乱択**は合法手から一様に選ぶだけの下限。
**素人筋**は初心者がまず思いつく方針 ── 「宇宙飛行士を多く出せるカードが良いカード」
「資源の高いエリア行きの船に乗せる」「排除するならいちばん数の多い相手」── だけで打ちます。
抜けているのは多数派の読みだけで、誰が何人いるか、自分が1人足したら取り分がどう動くかを
一切見ていません。その一点だけで .435 → .065 まで落ちます。

席順の有利不利を疑って、AI 4人だけでも400局回しました。
席別の勝率は **.205 / .275 / .253 / .268** で、偏りは信頼区間（±.043）の内側に収まっています。

対局は種から完全に再現します（同じ `--seed` なら1手まで同じ）。
`js/ai.js` に `Math.random()` が1か所だけ残っていたのを、この検証で見つけて直しました
── 発見カードの置き場所に迷ったときの最後の同点崩しで、
同じ種で回しても勝率が ±.003 ほど揺れていました。

```sh
node bench/run.js                                      # 上の一式をまとめて回す（約13秒）
node bench/run.js --lineup ai,naive,naive,naive --games 400
node bench/run.js --lineup ai,ai,ai,ai                 # 席順の偏りを見る
```

ベンチは `js/` をそのまま読み込んで動かします。**遊ぶときのコードと測るときのコードは同じ1つ**で、
測定用の複製は持ちません。比較用の乱択・素人筋だけが `bench/agents.js` にあります。

## 構成

ビルド手順のない素の HTML / CSS / JavaScript です。外部ライブラリは使っていません
（外部から読み込むのは下記のアクセス解析だけです）。
...
bench/load.js       js/ を Node から読み込む
bench/agents.js     比較用の相手（乱択・素人筋）
bench/run.js        勝率の実測
```
